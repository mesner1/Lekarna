...
import si.feri.praktikum.Zapis;
...
//					ResultSet rs2 = ps2.executeQuery();
					...
					ret = new Zapis(id, rs.getInt("kartoteka_id"), tip.getNaziv(), rs.getString("avtor"), cas);
					break;
				}
			} catch (Exception e) {
				e.printStackTrace();
    	...
    	public List<Zapis> vrniVse(int id) throws Exception {
			...
			try {
				...
				while (rs.next()) {
					...
					GregorianCalendar cas = new GregorianCalendar();
					cas.setTimeInMillis(rs.getTimestamp("cas").getTime());
					Zapis o = new Zapis(rs.getInt("kartoteka_id"), tip.getNaziv(), rs.getString("avtor"), dopolnila, cas);
