	 ...
	 private ZapisDAO() {
		...
		public void kreirajZapis() throws Exception {
			...
			try {
				...
				conn.createStatement().execute("create table dopolnilo (id int auto_increment not null, cas date not null, kartoteka_id integer not null, tip_id integer not null, avtor varchar(255), izdan integer, primary key (id))");
			} catch (Exception e) {
				e.printStackTrace();
			} finally {
				conn.close();
